...
function findLongestWord(sentence) {
  const longestWord = sentence
    .replace(/[^a-zA-Z\s]/g, '') // Убираем знаки пунктуации
    .split(/\s+/) // Разбиваем предложение на слова
    .reduce(
      (longest, current) =>
...
function encodeToRot13(str) {
  const arrAlpha = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const cipher = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(
    /[a-zA-Z]/gi,
    (letter) => cipher[arrAlpha.indexOf(letter)]
  );
}
